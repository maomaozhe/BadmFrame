...
```text
输入视频
  -> V1: 导入候选 JSON 或调用外部 runner
  -> 后续: 抽帧 / 主场地估计 / 轨迹推理 / 状态机
  -> 候选回合
  -> 人工审查 / 修正
  -> 确认后的候选转成普通 clips
  -> 复用现有导出流程
...
V1 先固定 rally candidate JSON/API 契约和 Web 审查闭环。模型层只通过
`BADMFRAME_RALLY_CANDIDATES_PATH` 指向的 JSON 或未来外部 runner 接入；
TrackNetV3、其他球轨迹模型和本地推理环境后续单独集成，不作为当前主服务
可运行前提。
...
V1 不在 FastAPI 主进程内集成 TrackNetV3。后端只依赖稳定边界：

- 输入：源视频路径和视频时长。
- 输出：标准 rally candidate JSON。
- 本地验证：`BADMFRAME_RALLY_CANDIDATES_PATH` 可指向 deterministic mock JSON。
- 未配置 runner 时必须明确失败，避免把空结果伪装成模型输出。
