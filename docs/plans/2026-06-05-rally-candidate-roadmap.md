...
> 历史计划说明：本文记录 2026-06-05 Web 验证链路阶段的实施计划。2026-06-09 后续方向已调整为移动端本地自动剪辑优先，Web 主要作为链路验证、算法评估和调试平台；有效回合边界误差目标收敛到小于 1s。当前事实以 `AGENTS.md`、`docs/product.md` 和 `docs/rally-segmentation.md` 为准。
...
**历史架构方向：** Web 在当时作为第一阶段用户可见的验证入口。后续方向已调整为移动端本地自动剪辑优先；模型/算法层仍通过标准 JSON 和 API 契约隔离，避免 Web 验证链路和移动端实现分叉。
...
- 首发载体：Web 审查台。
- 当前成功标准：大多数真实有效回合能被找到，候选边界误差目标小于 1s；暂未达到时用户必须能很快微调。
